...
feat: 融合双条带切割算法与Guillotine算法，用户可选
feat: 排版模式添加推荐场景说明
feat: 可拓展的配置文件存储 (ConfigManager)
"""
...
class LayoutWorker(QThread):
    """后台布局计算线程 - 支持双条带算法(默认)、Guillotine算法和贪心算法"""
    progress = Signal(int)
    ...
    def __init__(self, photo_sources, canvas_width, canvas_height, h_spacing_px, v_spacing_px, dpi=300,
                 left_margin=0, top_margin=0, layout_mode="double_strip"):
        super().__init__()
        ...
        self.layout_mode = layout_mode   # "double_strip", "guillotine", "greedy"
    ...
    def run(self):
        ...
        if self.layout_mode == "double_strip":
            pages = self.double_strip_packing(rects)
        elif self.layout_mode == "guillotine":
    ...
    def double_strip_packing(self, rects):
        ...
        while remaining:
            ...
            remaining = same_height + [r for r in remaining if r['slot_h'] != max_height]
            remaining.sort(key=lambda r: (-r['slot_h'], -r['slot_w']))
    ...
    def guillotine_packing(self, rects):
        ...
        # 按面积降序排序，优先放置大照片
        rects = sorted(rects, key=lambda x: x['slot_w'] * x['slot_h'], reverse=True)
    ...
    def greedy_packing(self, rects):
        ...
        current_page = []
        cut_lines = []
        rects = sorted(rects, key=lambda x: x['slot_h'], reverse=True)
        spaces = [(self.left_margin, self.top_margin, self.canvas_width, self.canvas_height)]
...
class ConfigManager:
    """可拓展的配置文件管理器"""
    CONFIG_FILE = "photo_layout_config.json"
    ...
    def set_section(self, name, data):
        ...
        self._dirty_sections.add(name)
        self.save()

    def has_section(self, name):
        return name in self._data

    def ensure_section(self, name, defaults):
        if name not in self._data or not isinstance(self._data[name], list):
            self._data[name] = list(defaults)
            self.save()
        return self._data[name]

    def update_section_item(self, section, index, data):
    ...
    def add_section_item(self, section, data):
        items = self._data.setdefault(section, [])
        items.append(data)
        self._dirty_sections.add(section)
        self.save()

    def remove_section_item(self, section, index):
        items = self._data.get(section)
        if items is not None and 0 <= index < len(items):
...
class SizeManager:
    ...
    def __init__(self, config_manager):
        ...
        self.photo_sizes = self.config.ensure_section("photo_sizes", self.DEFAULT_PHOTO_SIZES)
        self.canvas_sizes = self.config.ensure_section("canvas_sizes", self.DEFAULT_CANVAS_SIZES)
    
    def save_sizes(self):
        self.config.set_section("photo_sizes", self.photo_sizes)
        self.config.set_section("canvas_sizes", self.canvas_sizes)
        return True
...
size_manager = SizeManager(config_manager)
...
class EnhancedPhotoLayoutTool(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("证件照片排版工具v8.0.0")
        ...
        self._connect_layout_settings_save()
    ...
    def create_control_panel(self):
        ...
        self.layout_mode_combo.addItem("双条带算法 (混合尺寸排版整齐，裁切方便)", "double_strip")
        self.layout_mode_combo.addItem("Guillotine算法 (单一尺寸利用率较高，混合尺寸一般)", "guillotine")
        self.layout_mode_combo.addItem("贪心算法 (旧版)", "greedy")
        ...
        # 推荐说明标签（动态更新）
        self.mode_hint_label = QLabel("双条带算法：排版规整，裁切线整齐，适合使用切纸刀裁剪。")
        self.mode_hint_label.setStyleSheet("font-size: 9px; color: #909399; padding-left: 5px;")
    ...
    def on_layout_mode_changed(self, index):
        """更新排版模式说明文本"""
        mode = self.layout_mode_combo.currentData()
        if mode == "double_strip":
            self.mode_hint_label.setText("双条带算法：排版规整，裁切线整齐，适合混合尺寸、切纸刀裁剪。")
        elif mode == "guillotine":
            self.mode_hint_label.setText("Guillotine算法：单一尺寸时利用率较高；混合尺寸可能产生较多空白，请谨慎选择。")
        else:
            self.mode_hint_label.setText("贪心算法：简单快速，但利用率低，仅供兼容旧版使用。")
        self.update_preview()
    ...
    def apply_stylesheet(self):
        ...
        # 排版模式组合框样式
        self.layout_mode_combo.setStyleSheet(f"""
            QComboBox {{
                border: 1px solid {t['border']};
                border-radius: 3px;
                padding: 4px 6px;
        ...
        """)
        self.mode_hint_label.setStyleSheet(f"font-size: 9px; color: {t['text_secondary']}; padding-left: 5px;")
    ...
    def _connect_layout_settings_save(self):
        signals = [
            self.canvas_combo.currentIndexChanged,
            self.h_spacing.textChanged,
            self.v_spacing.textChanged,
            self.dpi_combo.currentIndexChanged,
            self.left_margin.textChanged,
            self.right_margin.textChanged,
            self.top_margin.textChanged,
            self.bottom_margin.textChanged,
            self.layout_mode_combo.currentIndexChanged,
            self.snap_checkbox.stateChanged,
            self.png_check.stateChanged,
            self.jpg_check.stateChanged,
            self.bmp_check.stateChanged,
            self.tiff_check.stateChanged,
        ]
        for sig in signals:
            sig.connect(self._save_layout_settings)
    
    def _save_layout_settings(self):
        config_manager.set_section("layout_settings", {
            "canvas_index": self.canvas_combo.currentIndex(),
            "h_spacing": self.h_spacing.text(),
            "v_spacing": self.v_spacing.text(),
            "dpi_index": self.dpi_combo.currentIndex(),
            "left_margin": self.left_margin.text(),
            "right_margin": self.right_margin.text(),
            "top_margin": self.top_margin.text(),
            "bottom_margin": self.bottom_margin.text(),
            ...
            "export_bmp": self.bmp_check.isChecked(),
            "export_tiff": self.tiff_check.isChecked(),
        })
    
    def _load_layout_settings(self):
        settings = config_manager.get_section("layout_settings")
        if not settings:
            return
        if "canvas_index" in settings:
            idx = settings["canvas_index"]
            if 0 <= idx < self.canvas_combo.count():
                self.canvas_combo.setCurrentIndex(idx)
        if "h_spacing" in settings:
            self.h_spacing.setText(settings["h_spacing"])
        if "v_spacing" in settings:
            self.v_spacing.setText(settings["v_spacing"])
        if "dpi_index" in settings:
            idx = settings["dpi_index"]
            if 0 <= idx < self.dpi_combo.count():
                self.dpi_combo.setCurrentIndex(idx)
        if "left_margin" in settings:
            self.left_margin.setText(settings["left_margin"])
        if "right_margin" in settings:
            self.right_margin.setText(settings["right_margin"])
        if "top_margin" in settings:
            self.top_margin.setText(settings["top_margin"])
        if "bottom_margin" in settings:
            self.bottom_margin.setText(settings["bottom_margin"])
        if "layout_mode" in settings:
            mode = settings["layout_mode"]
            idx = self.layout_mode_combo.findData(mode)
            if idx >= 0:
                self.layout_mode_combo.setCurrentIndex(idx)
        if "snap_enabled" in settings:
            self.snap_checkbox.setChecked(settings["snap_enabled"])
        if "export_png" in settings:
            self.png_check.setChecked(settings["export_png"])
        if "export_jpg" in settings:
            self.jpg_check.setChecked(settings["export_jpg"])
        if "export_bmp" in settings:
            self.bmp_check.setChecked(settings["export_bmp"])
        if "export_tiff" in settings:
            self.tiff_check.setChecked(settings["export_tiff"])
    ...
    def auto_layout(self):
        ...
        self.layout_worker = LayoutWorker(
            ...
            left_margin_px, top_margin_px, layout_mode
        )
