...
class PhotoItem(QGraphicsPixmapItem):
    """画布上的照片项，支持拖拽和吸附对齐"""
    def __init__(self, photo_source, width_px, height_px, snap_enabled=True, parent=None):
...
class SnapPhotoItem(PhotoItem):
    ...
    def __init__(self, photo_source, width_px, height_px, snap_enabled=True, parent=None):
        super().__init__(photo_source, width_px, height_px, snap_enabled, parent)
...
class LayoutWorker(QThread):
    ...
    def __init__(self, photo_sources, canvas_width, canvas_height, spacing_px, dpi=300, left_margin=0, top_margin=0, copy_shop_mode=False):
        super().__init__()
        ...
        self.copy_shop_mode = copy_shop_mode
    
    def run(self):
        """执行布局计算，支持多页"""
        if self.copy_shop_mode:
            self.run_copy_shop_layout()
        else:
            self.run_greedy_layout()
    
    def run_copy_shop_layout(self):
        """复印店模式：严格的网格排列，充分利用空间减少空白"""
        pages = []
        
        # 收集所有照片的尺寸信息，并按尺寸分组
        size_groups = {}  # {(width, height): [photos]}
        
        for i, photo in enumerate(self.photo_sources):
            size_info = size_manager.get_photo_size(photo.target_size_index)
            if size_info:
                w_px = self.cm_to_px(size_info["width"])
                h_px = self.cm_to_px(size_info["height"])
                key = (w_px, h_px)
        ...
        # 按尺寸面积从大到小排序，优先排大照片
        sorted_sizes = sorted(size_groups.keys(), key=lambda k: k[0] * k[1], reverse=True)
        
        # 为每种尺寸计算网格参数
        size_grid_info = {}
        for size_key in sorted_sizes:
            w, h = size_key
            item_width = w + self.spacing_px
            item_height = h + self.spacing_px
            cols = max(1, (self.canvas_width - self.left_margin) // item_width)
            rows = max(1, (self.canvas_height - self.top_margin) // item_height)
            size_grid_info[size_key] = {
        ...
        # 分配照片到页面
        page_data = []  # [{'photos': [], 'size_key': (w, h)}]
        
        for size_key in sorted_sizes:
            photos = size_groups[size_key]
            grid_info = size_grid_info[size_key]
            items_per_page = grid_info['items_per_page']
            ...
            for photo in photos:
                ...
                for page in page_data:
                    # 如果当前页的照片尺寸相同且还有空间
                    if page['size_key'] == size_key and len(page['photos']) < items_per_page:
        ...
        for page_idx, page in enumerate(page_data):
            size_key = page['size_key']
            photos = page['photos']
            grid_info = size_grid_info[size_key]
            
            current_page = []
            max_cols = grid_info['cols']
            item_width = grid_info['item_width']
            item_height = grid_info['item_height']
            photo_width = grid_info['width']
            photo_height = grid_info['height']
            
            # 根据照片数量计算最佳列数，使右边空白最少
            n = len(photos)
            if n > 0:
                # 尝试不同的列数，找出使右边空白最小的方案
                best_cols = 1
                min_right_space = float('inf')
                
                for cols in range(1, max_cols + 1):
                    rows = (n + cols - 1) // cols  # 向上取整
                    # 计算总宽度
                    total_width = cols * item_width
                    # 计算右边空白
                    right_space = self.canvas_width - self.left_margin - total_width
                    if right_space >= 0 and right_space < min_right_space:
                        min_right_space = right_space
                        best_cols = cols
                
                actual_cols = best_cols
            else:
                actual_cols = 1
            ...
            for idx, photo in enumerate(photos):
                ...
                x = self.left_margin + col * item_width
                y = self.top_margin + row * item_height
                ...
                current_page.append({
                    ...
                    'width': photo_width,
                    'height': photo_height,
                    'source_index': photo['source_index'],
                    'rotation': False
                })
            
            pages.append(current_page)
            self.progress.emit(30 + int((page_idx + 1) / max(total_pages, 1) * 70))
        
        self.finished.emit(pages)
    
    def run_greedy_layout(self):
        """贪心算法布局（原有逻辑）"""
        pages = []  # 存储每一页的布局
        current_page = []
        ...
        for rect in rects:
            ...
            if best_space:
                ...
                if spaces[0][2] >= w and spaces[0][3] >= h:
                    ...
                    # 更新空间（考虑边距）
                    right_space = (self.left_margin + w, self.top_margin, self.canvas_width - w - self.left_margin, self.canvas_height - self.top_margin)
                    bottom_space = (self.left_margin, self.top_margin + h, self.canvas_width - self.left_margin, self.canvas_height - h - self.top_margin)
                    spaces = []
                    if right_space[2] > 10 and right_space[3] > 10:
...
class EnhancedPhotoLayoutTool(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("专业证件照片排版工具v7.6")
        ...
        self.copy_shop_mode = True  # 复印店模式默认开启
    ...
    def create_control_panel(self):
        ...
        # 复印店模式
        copy_shop_group = QGroupBox("复印店模式")
        copy_shop_layout = QVBoxLayout(copy_shop_group)
        
        self.copy_shop_check = QCheckBox("启用复印店模式")
        self.copy_shop_check.setChecked(True)
        self.copy_shop_check.setStyleSheet("""
            QCheckBox { font-size: 12px; font-weight: bold; color: #303133; }
            QCheckBox::indicator { width: 18px; height: 18px; }
        """)
        self.copy_shop_check.stateChanged.connect(self.on_copy_shop_mode_changed)
        copy_shop_layout.addWidget(self.copy_shop_check)
        
        copy_shop_hint = QLabel("横平竖直的网格排列，一刀裁切，方便快捷")
        copy_shop_hint.setStyleSheet("font-size: 10px; color: #909399; padding-left: 5px;")
        copy_shop_layout.addWidget(copy_shop_hint)
        
        layout.addWidget(copy_shop_group)
    ...
    def auto_layout(self):
        ...
        self.layout_worker = LayoutWorker(
            self.photo_sources, effective_canvas_w, effective_canvas_h, spacing_px, dpi,
            left_margin_px, top_margin_px, self.copy_shop_mode
        )
        ...
        for page_items in pages:
            ...
            for item_data in page_items:
                ...
                if source_idx < len(self.photo_sources):
                    ...
                    photo_item = SnapPhotoItem(photo, w_px, h_px, self.snap_enabled)
                    photo_item.setPos(item_data['x'], item_data['y'])
                    
                    # 复印店模式下禁用拖拽
                    if self.copy_shop_mode:
                        photo_item.setFlag(QGraphicsItem.GraphicsItemFlag.ItemIsMovable, False)
    ...
    def on_copy_shop_mode_changed(self, state):
        """复印店模式开关状态改变"""
        self.copy_shop_mode = (state == 2)
        # 复印店模式下自动启用吸附
        if self.copy_shop_mode:
            self.snap_enabled = True
            self.snap_checkbox.setChecked(True)
            self.snap_checkbox.setEnabled(False)
        else:
            self.snap_checkbox.setEnabled(True)
        self.statusBar.showMessage(f"复印店模式: {'开启' if self.copy_shop_mode else '关闭'}")
