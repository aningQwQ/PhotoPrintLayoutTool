...
class PhotoItem(QGraphicsPixmapItem):
    """画布上的照片项，支持拖拽和旋转"""
    def __init__(self, photo_source, width_px, height_px, rotation=0, parent=None):
        ...
        # 应用旋转
        if rotation == 90:
            self.setTransform(QTransform().rotate(90))
...
class LayoutWorker(QThread):
    ...
    skipped = Signal(list)  # 返回跳过的照片列表
    
    def __init__(self, photo_sources, canvas_width, canvas_height, spacing_px, dpi=300):
        super().__init__()
    ...
    def run(self):
        """执行布局计算，支持多页、智能旋转、居中留白"""
        pages = []  # 存储每一页的布局
        current_page = []
        skipped_photos = []  # 记录因尺寸过大而跳过的照片
        
        # 边距（毫米转像素）
        margin_mm = 3  # 默认3mm边距
        margin_px = self.cm_to_px(margin_mm / 10)
        
        # 有效画布区域（去掉边距）
        canvas_w = self.canvas_width - 2 * margin_px
        canvas_h = self.canvas_height - 2 * margin_px
        ...
        for i, photo in enumerate(self.photo_sources):
            ...
            if size_info:
                ...
                # 检查原方向是否超出画布
                w_px_orig = self.cm_to_px(w)
                h_px_orig = self.cm_to_px(h)
                
                # 检查旋转后是否超出画布
                w_px_rot = self.cm_to_px(h)
                h_px_rot = self.cm_to_px(w)
                
                can_fit_orig = (w_px_orig <= canvas_w and h_px_orig <= canvas_h)
                can_fit_rot = (w_px_rot <= canvas_w and h_px_rot <= canvas_h)
                
                if not can_fit_orig and not can_fit_rot:
                    # 两种方向都无法放入
                    skipped_photos.append((photo.file_name, size_info["name"], w, h))
                    continue
                
                # 选择更好的_area = w_方向
                origpx_orig * h_px_orig
                rot_area = w_px_rot * h_px_rot
                
                # 如果原方向放不下，但旋转后可以，选择旋转
                # 如果两者都能放下，选择能放入剩余空间更小的方向
                if not can_fit_orig and can_fit_rot:
                    rotated = True
                    w_px = w_px_rot
                    h_px = h_px_rot
                elif can_fit_orig and can_fit_rot:
                    # 智能选择：选择使得剩余空间更小的方向
                    # 这有助于减少碎片，提高利用率
                    rotated = False  # 默认不旋转
                else:
                ...
                for q in range(photo.quantity):
                    rects.append({
                        'w': w_px + self.spacing_px,
                        'h': h_px + self.spacing_px,
                        'source_idx': i,
                        'qty_idx': q,
                        'rotated': rotated,
                        'orig_w': w_px,
        ...
        if skipped_photos:
            # 发送跳过的照片信息
            self.skipped.emit(skipped_photos)
        
        if not rects:
            self.finished.emit([])
            return
        
        # 使用贪心算法进行多页装箱（按高度降序排序）
        rects = sorted(rects, key=lambda x: x['h'], reverse=True)
        
        # 可用空间列表
        spaces = [(margin_px, margin_px, canvas_w, canvas_h)]
        
        for rect in rects:
            w, h = rect['w'], rect['h']
            source_idx = rect['source_idx']
            rotated = rect['rotated']
            orig_w, orig_h = rect['orig_w'], rect['orig_h']
            ...
            if best_space:
                ...
                for i, s in enumerate(spaces):
                    if i == best_idx:
                        right_x = best_space[0] + w
                        right_w = best_space[2] - w
                        bottom_y = best_space[1] + h
                        bottom_h = best_space[3] - h
                        
                        # 只有当分割后的空间足够大时才添加
                        if right_w > 5 and best_space[3] > 5:
                            new_spaces.append((right_x, best_space[1], right_w, best_space[3]))
                        if best_space[2] > 5 and bottom_h > 5:
                            new_spaces.append((best_space[0], bottom_y, best_space[2], bottom_h))
                    else:
                        new_spaces.append(s)
                spaces = new_spaces
                ...
                if spaces[0][2] >= w and spaces[0][3] >= h:
                    ...
                    # 更新空间
                    right_space = (margin_px + w, margin_px, canvas_w - w, canvas_h)
                    bottom_space = (margin_px, margin_px + h, w, canvas_h - h)
                    spaces = []
                    if right_space[2] > 5 and right_space[3] > 5:
                        spaces.append(right_space)
                    if bottom_space[2] > 5 and bottom_space[3] > 5:
...
class EnhancedPhotoLayoutTool(QMainWindow):
    ...
    def update_preview(self):
        ...
        # 边距（3mm）
        margin_mm = 3
        margin_px = int(margin_mm / 10 / 2.54 * dpi)
        ...
        # 绘制灰色背景（打印区域外）
        outer_bg = self.scene.addRect(0, 0, canvas_w_px, canvas_h_px, 
                                      QPen(Qt.GlobalColor.transparent), 
                                      QBrush(QColor(80, 80, 80)))
        
        # 绘制白色背景（有效打印区域）
        bg = self.scene.addRect(margin_px, margin_px, 
                                canvas_w_px - 2*margin_px, canvas_h_px - 2*margin_px, 
                                QPen(Qt.GlobalColor.transparent), 
                                QBrush(Qt.GlobalColor.white))
        
        # 绘制边框
        border = self.scene.addRect(margin_px, margin_px, 
                                   canvas_w_px - 2*margin_px, canvas_h_px - 2*margin_px,
                                   QPen(QColor(180, 190, 210), 2, Qt.PenStyle.DashLine),
    ...
    def auto_layout(self):
        ...
        self.layout_worker = LayoutWorker(
            self.photo_sources, canvas_w_px, canvas_h_px, spacing_px, dpi
        )
        self.layout_worker.progress.connect(self.on_layout_progress)
        self.layout_worker.skipped.connect(self.on_layout_skipped)
    ...
    def on_layout_skipped(self, skipped_list):
        """处理尺寸过大被跳过的照片"""
        if skipped_list:
            msg = "以下照片因尺寸超过画布已被跳过：\n\n"
            for filename, size_name, w, h in skipped_list:
                msg += f"• {filename} ({size_name}: {w}cm×{h}cm)\n"
            QMessageBox.warning(self, "尺寸超出", msg)
    ...
    def on_layout_finished(self, pages):
        ...
        for page_items in pages:
            ...
            # 绘制白色背景（有效打印区域）
            page_scene.addRect(margin_px, margin_px, 
                             canvas_w_px - 2*margin_px, canvas_h_px - 2*margin_px, 
                             QPen(Qt.GlobalColor.transparent), 
                             QBrush(Qt.GlobalColor.white))
            ...
            for item_data in page_items:
                ...
                if source_idx < len(self.photo_sources):
                    ...
                    photo_item = PhotoItem(photo, w_px, h_px, rotation)
