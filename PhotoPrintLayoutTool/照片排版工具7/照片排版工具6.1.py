...
class EnhancedPhotoLayoutTool(QMainWindow):
    def __init__(self):
        ...
        subtitle_label = QLabel(f"配置文件: {self.size_manager.config_file}")
        ...
        self.orientation_group.idClicked.connect(self.update_orientation)
        ...
        # 创建一个水平布局用于放置标题和许可证信息
        title_layout = QHBoxLayout()
        
        # 标题
        preview_title = QLabel("排版预览")
        preview_title.setStyleSheet("font-size: 18px; font-weight: bold; color: #303133;")
        title_layout.addWidget(preview_title, alignment=Qt.AlignmentFlag.AlignLeft)
        
        # 许可证信息（使用与statsLabel相同的样式）
        license_label = QLabel("MIT License © 2025 徐英珺")
        license_label.setStyleSheet("font-size: 11px; color: #909399;")
        title_layout.addWidget(license_label, alignment=Qt.AlignmentFlag.AlignRight)
        ...
        self.stats_label1.setAlignment(Qt.AlignmentFlag.AlignCenter)
        ...
        self.stats_label3.setAlignment(Qt.AlignmentFlag.AlignCenter)
        
        self.stats_label4 = QLabel("方向: -")
        self.stats_label4.setObjectName("statsLabel")
        self.stats_label4.setAlignment(Qt.AlignmentFlag.AlignCenter)
    ...
    def update_orientation(self, id):
        """更新纸张方向"""
        self.orientation_mode = id
        self.update_preview()
    ...
    def update_preview(self):
        ...
        # 创建预览图像
        preview_img = QImage(canvas_w, canvas_h, QImage.Format.Format_RGB32)
        preview_img.fill(QColor(235, 238, 245))  # 预览背景色
        
        # 确保 QPainter 在有效设备上开始和结束
        painter = QPainter(preview_img)
        try:
            painter.setRenderHint(QPainter.RenderHint.Antialiasing)
            
            # 绘制画布边框
            painter.setPen(QPen(QColor(180, 190, 210), 3, Qt.PenStyle.DashLine))
            ...
            if self.photo_pixmap and not self.photo_pixmap.isNull():
                ...
                scaled_photo = self.photo_pixmap.scaled(
                    photo_w, photo_h, Qt.AspectRatioMode.IgnoreAspectRatio, Qt.TransformationMode.SmoothTransformation
                )
                # 在第一个位置绘制照片预览
                first_x = margin_x
                first_y = margin_y
    ...
    def generate_layout(self):
        ...
        # 创建最终图像
        result_img = QImage(canvas_w, canvas_h, QImage.Format.Format_RGB32)
        result_img.fill(Qt.GlobalColor.white)  # 白色背景
