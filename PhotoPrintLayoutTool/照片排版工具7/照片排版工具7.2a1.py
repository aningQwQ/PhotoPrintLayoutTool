...
class PhotoSource:
    ...
    def __init__(self, file_path, pixmap):
        ...
        self.fit_mode = "fill"  # "fill"填满裁切 / "crop"适应裁切 / "stretch"变形 / "keep"保持不变
...
class PhotoItem(QGraphicsPixmapItem):
    ...
    def __init__(self, photo_source, width_px, height_px, parent=None):
        ...
        # 根据fit_mode缩放照片
        if photo_source.fit_mode == "fill":
            # 填满裁切模式：填满整个区域，从中心裁切多余部分
            ...
            # 从中心裁切
            x_offset = (width_px - scaled.width()) // 2
            y_offset = (height_px - scaled.height()) // 2
            self.setOffset(x_offset, y_offset)
        elif photo_source.fit_mode == "crop":
            # 适应裁切模式：保持比例，自动缩放（可能有留白）
            scaled = photo_source.original_pixmap.scaled(
                width_px, height_px, 
            ...
            y_offset = (height_px - scaled.height()) // 2
            self.setOffset(x_offset, y_offset)
        elif photo_source.fit_mode == "keep":
            # 保持不变模式：使用原始尺寸
            scaled = photo_source.original_pixmap
            # 居中显示
            x_offset = (width_px - scaled.width()) // 2
            y_offset = (height_px - scaled.height()) // 2
            self.setOffset(x_offset, y_offset)
...
class PhotoItemWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.size_combo.setStyleSheet("QComboBox { font-size: 9px; min-height: 18px; }")
        ...
        # 数量
        qty_label = QLabel("数量:")
        qty_label.setStyleSheet("font-size: 9px;")
        ...
        self.qty_spin.setFixedWidth(35)
        self.qty_spin.setStyleSheet("QSpinBox { font-size: 9px; min-height: 18px; }")
        ...
        self.fill_radio = QRadioButton("填满")
        self.fill_radio.setStyleSheet("font-size: 8px;")
        self.fill_radio.setChecked(self.photo_source.fit_mode == "fill")
        
        self.crop_radio = QRadioButton("适应")
        self.crop_radio.setStyleSheet("font-size: 8px;")
        self.crop_radio.setChecked(self.photo_source.fit_mode == "crop")
        
        self.keep_radio = QRadioButton("保持")
        self.keep_radio.setStyleSheet("font-size: 8px;")
        self.keep_radio.setChecked(self.photo_source.fit_mode == "keep")
        
        self.stretch_radio = QRadioButton("变形")
        self.stretch_radio.setStyleSheet("font-size: 8px;")
        self.stretch_radio.setChecked(self.photo_source.fit_mode == "stretch")
        
        row2.addWidget(self.fill_radio)
        row2.addWidget(self.crop_radio)
        row2.addWidget(self.keep_radio)
    ...
    def on_delete_clicked(self):
        ...
        reply = QMessageBox.question(
            self, "确认删除", 
            f"确定要删除照片\n{self.photo_source.file_name}\n吗？",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            QMessageBox.StandardButton.No
        )
        if reply == QMessageBox.StandardButton.Yes:
            if self.delete_clicked:
                self.delete_clicked(self.photo_source)
...
class EnhancedPhotoLayoutTool(QMainWindow):
    ...
    def add_photos(self, file_paths):
        ...
        for file_path in file_paths:
            ...
            item_widget.delete_clicked = lambda ps: self.delete_photo(ps)
    ...
    def delete_photo(self, photo_source):
        """删除单张照片"""
        if photo_source in self.photo_sources:
            ...
            for i in range(self.photo_list.count()):
                ...
                if widget and widget.photo_source == photo_source:
                    self.photo_list.takeItem(i)
                    break
            self.update_stats()
            self.statusBar.showMessage(f"已删除照片")
