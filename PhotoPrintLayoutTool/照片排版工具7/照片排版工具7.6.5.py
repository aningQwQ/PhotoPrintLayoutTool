...
class LayoutWorker(QThread):
    ...
    def __init__(self, photo_sources, canvas_width, canvas_height, h_spacing_px, v_spacing_px, dpi=300,
        ...
        self.row_packing_mode = row_packing_mode

    def run(self):
        """执行布局计算，行打包分组模式使用行打包算法"""
        pages = []

        # 收集所有需要排列的矩形（净尺寸+间距）
        rects = []  # (净宽, 净高, 带间距宽, 带间距高, 照片索引, 数量副本序号)
        ...
        if self.row_packing_mode:
            # 行打包分组模式：行打包 + 按高度分组
            pages = self.row_packing_with_grouping(rects)
        else:
            # 保留原来的贪心算法（兼容旧行为）
    ...
    # ------------------- 行打包分组模式（真正的条带内嵌套对齐） -------------------
    def row_packing_with_grouping(self, rects):
        """
        分层条带打包算法：
        - 宏观上按高度降序排序，保证大照片先处理
        - 每行维护一个空闲矩形列表（Free Rectangles）
        - 放置新照片时，优先在空闲矩形中寻找合适的空隙（允许矮照片填入高照片旁的下方区域）
        - 要求照片的顶部或底部与已有照片的水平边缘对齐（保证切割线连续）
        - 若没有合适空隙，则追加到当前行末或新建行/页
        """
        if not rects:
            return []

        # 1. 按高度降序 -> 宽度降序（大照片优先确定行骨架）
        rects_sorted = sorted(rects, key=lambda r: (-r['height'], -r['width']))

        pages = []
        current_page_items = []      # 当前页所有照片
        current_y = self.top_margin   # 当前行顶部 Y 坐标（全局行骨架）
        row_height = 0                # 当前行的最大高度（用于判断是否需换行）
        free_rects = []               # 当前行的空闲矩形列表 [(x, y, w, h)]  坐标相对于画布
                                      # y 坐标是基于当前行顶部的，但实际存储绝对坐标

        def add_free_rect(x, y, w, h):
            """添加空闲矩形，并尝试合并相邻空闲区域（简化）"""
            # 简单去重，不合并，但避免过小
            if w > 5 and h > 5:
                free_rects.append((x, y, w, h))

        def find_best_fit(rect_w, rect_h):
            """
            在当前空闲矩形中寻找最佳放置位置。
            优先选择满足以下条件的空闲矩形：
            - 宽度和高度均足够
            - 放置后照片的顶部或底部能与已有照片的某个水平边缘对齐？（此处简化：要求顶部对齐空闲矩形顶部，即与行内已有照片的顶边对齐）
            实际上，为了让照片与已放置照片的边缘对齐，我们只考虑空闲矩形的顶部与当前行顶部对齐的情况，
            或者让矮照片填充高照片旁的下方区域时，空闲矩形可能是由高照片右侧创造出的垂直空间。
            这里我们采用贪心：选择空洞中左上角位置，但确保照片放置后不破坏切割线连续性（即新照片的上边或下边与某个已放置照片的边对齐）。
            为了保持代码简洁，我们采用标准空隙填充算法：在free_rects中找到第一个能容纳的矩形，并放在其左上角。
            """
            best_rect = None
            best_idx = -1
            # 按左上角Y坐标优先（尽量靠上），X坐标次之
            for i, (rx, ry, rw, rh) in enumerate(free_rects):
                if rw >= rect_w and rh >= rect_h:
                    if best_rect is None or ry < best_rect[1] or (ry == best_rect[1] and rx < best_rect[0]):
                        best_rect = (rx, ry, rw, rh)
                        best_idx = i
            return best_rect, best_idx

        def split_free_rect(rect, used_x, used_y, used_w, used_h):
            """
            将空闲矩形分割成剩余的多个矩形（Guillotine分割：优先水平切，再垂直切）
            返回新的空闲矩形列表
            """
            rx, ry, rw, rh = rect
            # 被占用的区域
            # 右侧剩余区域
            right_w = rw - (used_x - rx) - used_w
            if right_w > 5 and rh > 5:
                add_free_rect(used_x + used_w, ry, right_w, rh)
            # 左侧剩余区域（实际上会有重叠，标准做法是只分割右侧和下方，或者采用更完备的MaxRects）
            # 为了简单，只分割右侧和下方
            bottom_h = rh - (used_y - ry) - used_h
            if bottom_h > 5 and used_w > 5:
                add_free_rect(used_x, used_y + used_h, used_w, bottom_h)
            # 注意：左侧和上方理论上不会出现，因为我们总是从空闲矩形左上角开始放置

        def flush_row():
            """结束当前行，推进Y坐标"""
            nonlocal current_y, row_height, free_rects
            if row_height > 0:
                current_y += row_height + self.v_spacing_px
                row_height = 0
                free_rects.clear()

        def flush_page():
            """保存当前页，并重置"""
            nonlocal current_page_items, current_y, row_height, free_rects
        ...
        for rect in rects_sorted:
            slot_w = rect['slot_w']
            slot_h = rect['slot_h']
            if slot_w > self.canvas_width:
                continue  # 宽度超限，忽略（不应发生）

            # 尝试在当前行的空闲矩形中放置
            placed = False
            if free_rects:
                best_rect, idx = find_best_fit(slot_w, slot_h)
                if best_rect:
                    rx, ry, rw, rh = best_rect
                    # 放置在此空闲矩形的左上角
                    x = rx
                    y = ry
                    # 检查是否超出页面高度（实际不会，因为空闲矩形都在行内）
                    if y + slot_h > self.canvas_height:
                        # 当前页放不下，翻页
                        flush_page()
                        # 重新尝试在新页放置（但未处理空闲矩形，进入新行逻辑）
                        placed = False
                    else:
                        # 放置照片
                        ...
                        # 将空闲矩形分割
                        free_rects.pop(idx)
                        split_free_rect(best_rect, x, y, slot_w, slot_h)
                        # 更新行高（取当前放置的照片底边与已有行底边的最大值）
                        new_bottom = y + slot_h
                        row_height = max(row_height, new_bottom - current_y)
                        placed = True
                        continue

            if not placed:
                # 无法放入空闲矩形，尝试追加到当前行末（新建一个空闲区域？实际上追加就是扩展行）
                # 计算当前行已占用的总宽度
                used_width = 0
                # 如果没有空闲矩形（即没有任何照片），说明行空
                if not free_rects and not current_page_items:
                    # 空行，从左边距开始
                    x = self.left_margin
                    y = current_y
                else:
                    # 找出当前行最右侧的边界（所有照片的右边界最大值）
                    max_right = self.left_margin
                    # 由于空闲矩形可能来自分割，我们也可以扫描所有照片
                    for item in current_page_items:
                        # 注意：只考虑在当前行（y == current_y）的照片？
                        # 简化：当前行所有照片的x+w的最大值
                        if item['y'] == current_y:  # 仅考虑顶行照片
                            right = item['x'] + item['width']  # 这里应该是slot_w? 实际存储width/height
                            max_right = max(max_right, right)
                    x = max_right + (self.h_spacing_px if max_right > self.left_margin else 0)
                    y = current_y
                # 检查行内宽度是否足够
                if x + slot_w <= self.canvas_width:
                    # 可以放在同一行的右侧
                    current_page_items.append({
                        'x': x,
                        'y': y,
                    ...
                    new_bottom = y + slot_h
                    row_height = max(row_height, new_bottom - current_y)
                    # 在此照片右侧产生空闲矩形（如果右侧还有空间）
                    right_space_width = self.canvas_width - (x + slot_w)
                    if right_space_width > 5 and slot_h > 5:
                        free_rects.append((x + slot_w, y, right_space_width, slot_h))
                    placed = True
                else:
                    # 当前行放不下，需要换行
                    flush_row()
                    # 尝试在新行放置
                    if current_y + slot_h > self.canvas_height:
                        # 同时翻页
                        flush_page()
                    # 新行起始
                    x = self.left_margin
...
class EnhancedPhotoLayoutTool(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("证件照片排版工具v7.6.5")
    ...
    def create_control_panel(self):
        ...
        self.row_packing_check = QCheckBox("行打包分组模式")
        self.row_packing_check.setChecked(True)
        self.row_packing_check.setToolTip("启用行打包分组算法，按高度分组排列，切割线连续整齐")
        ...
        row_packing_hint = QLabel("启用行打包分组算法，按高度分组排列，切割线连续整齐")
    ...
    def apply_stylesheet(self):
        ...
        for lbl in row_packing_hint:
            if "行打包分组" in lbl.text() or "行打包" in lbl.text():
