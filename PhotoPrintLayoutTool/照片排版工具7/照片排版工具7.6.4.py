...
-- 修改高级装箱算法，改为条带打包，切割线连续，适合多尺寸混合排版
"""
...
class LayoutWorker(QThread):
    ...
    def row_packing_with_grouping(self, rects):
        """
        行打包策略（类似 Strip Packing）：
        - 按照片高度降序排序，同一高度内按宽度降序
        - 尽可能将同类尺寸放在同一行
        - 一行放不下则换行，换行后仍放不下则新建页面
        - 产生整齐的网格结构，切割线为直线，无碎片空白
        """
        if not rects:
            return []
        ...
        pages = []
        current_page_items = []      # 当前页的照片布局
        current_y = self.top_margin   # 当前行顶部Y坐标（含上边距）
        row_height = 0                # 当前行的最大高度（含垂直间距）
        row_items = []                # 当前行的照片列表

        # 辅助函数：结束当前行，推进Y坐标
        def finish_row():
            nonlocal current_y, row_height, row_items
            if row_items:
                # 当前行的高度取该行所有照片的最大高度（含垂直间距）
                # 注意：垂直间距只在行与行之间添加，最后一行不加
                current_y += row_height + self.v_spacing_px
                row_items.clear()
                row_height = 0

        # 辅助函数：将当前页加入pages，并重置
        def flush_page():
            nonlocal current_page_items, current_y, row_height, row_items
        ...
        for rect in rects_sorted:
            # 检查是否需要翻页（当前行放不下且当前行已有内容）
            if rect['slot_w'] > self.canvas_width:
                # 照片本身超过画布宽度，跳过（理论上不会发生，因为尺寸管理器限制了）
                continue

            # 尝试放入当前行
            if row_items:
                # 计算当前行已占用的总宽度
                used_width = sum(item['slot_w'] for item in row_items)
                if used_width + rect['slot_w'] <= self.canvas_width:
                    # 可以放在当前行末尾
                    x_pos = self.left_margin + used_width
                    ...
                    row_height = max(row_height, rect['slot_h'])
                    continue
                else:
                    # 当前行放不下，结束当前行
                    finish_row()
            ...
            if rect['slot_w'] <= self.canvas_width:
                # 检查Y坐标是否超出页面高度
                if current_y + rect['slot_h'] > self.canvas_height:
                    # 当前页放不下，翻页
                    flush_page()
                    # 重置到新页起始位置
                    current_y = self.top_margin
                ...
                row_height = rect['slot_h']
                # 不立即调用 finish_row，让后续照片继续尝试同一行
            else:
                # 宽度超限（不应该发生）
                continue
