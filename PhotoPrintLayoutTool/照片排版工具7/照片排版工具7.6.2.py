...
bug-修复linux主题导致的显示问题
feat-新增深色模式支持，优化界面适配
feat-跟随系统主题自动切换浅色/深色模式
"""
...
class LayoutWorker(QThread):
    ...
    def guillotine_packing(self, rects):
        ...
        # 安全限制：防止无限循环
        max_iterations = len(rects) * 10  # 允许的最大迭代次数
        iteration_count = 0
        skipped_count = 0  # 连续跳过的次数
...
class ZoomableGraphicsView(QGraphicsView):
    ...
    def __init__(self, parent=None):
        ...
        # 设置深色背景（默认浅色模式为#323232，切换深色模式时由主窗口更新）
...
class EnhancedPhotoLayoutTool(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("证件照片排版工具v7.6.1")
    ...
    def _detect_system_dark_mode():
        ...
        通过分析系统调色板中窗口背景色的亮度来判断。
        若亮度低于阈值（128），则认为系统处于深色模式。
        """
        try:
            ...
            # 计算亮度: 0.299*R + 0.587*G + 0.114*B (ITU-R BT.601)
            brightness = (
    ...
    def create_control_panel(self):
        ...
        self.dark_mode_btn.setObjectName("darkModeBtn")
    ...
    def toggle_dark_mode(self, checked):
        ...
        self.canvas_view.setBackgroundBrush(QBrush(theme["view_bg"]))
        ...
        # 更新已排版的页面
        if self.page_scenes:
            self.refresh_page_scenes()
        
        self.statusBar.showMessage(f"已切换至{'深色' if self.dark_mode else '浅色'}模式")
    ...
    def apply_stylesheet(self):
        ...
        # 更新深色模式按钮样式
        self.dark_mode_btn.setStyleSheet(f"""
            QPushButton {{
                background-color: {t['disabled']};
                color: {'#1e1e1e' if self.dark_mode else 'white'};
                border: none;
                border-radius: 4px;
                padding: 4px 10px;
                font-size: 11px;
            ...
            QPushButton:checked {{
                background-color: {'#1e1e1e' if self.dark_mode else '#606266'};
                color: {t['text_primary']};
                border: 1px solid {t['border']};
            }}
        """)
    ...
    def update_preview(self):
        ...
        # 根据当前主题选择画布背景色
        if self.dark_mode:
            scene_bg_color = QColor(45, 45, 45)  # 深色画布
    ...
    def refresh_page_scenes(self):
        ...
        for page_scene in self.page_scenes:
            ...
            for item in page_scene.items():
                # 只移除矩形背景元素（非照片项）
                if isinstance(item, type(page_scene.addRect(0, 0, 1, 1, QPen(Qt.GlobalColor.transparent), QBrush(Qt.GlobalColor.white)).__class__)):
...
if __name__ == "__main__":
    ...
    # 强制使用Fusion风格，确保CSS样式在各平台表现一致
    from PySide6.QtWidgets import QStyleFactory
    if "Fusion" in QStyleFactory.keys():
        app.setStyle("Fusion")
    
    # 全局滚动条样式（窗口级样式表不覆盖滚动条）
    app.setStyleSheet("""
        QScrollBar:vertical {
            background: #f0f0f0;
            width: 10px;
