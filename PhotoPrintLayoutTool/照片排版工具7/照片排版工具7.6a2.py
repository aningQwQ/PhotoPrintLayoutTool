...
class LayoutWorker(QThread):
    ...
    def __init__(self, photo_sources, canvas_width, canvas_height, spacing_px, dpi=300, left_margin=0, top_margin=0, guillotine_mode=True):
        super().__init__()
        ...
        for idx, rect in enumerate(rects):
            ...
            if best_space:
                ...
                current_page.append({
                    'x': best_space[0],
                    'y': best_space[1],
                    'width': actual_w - self.spacing_px,
                    'height': actual_h - self.spacing_px,
                    'source_index': source_idx,
                    'rotation': rotated
                })
                
                # 记录裁切线（Guillotine模式：记录分割线）
                cut_x = best_space[0] + actual_w
                cut_y = best_space[1] + actual_h
