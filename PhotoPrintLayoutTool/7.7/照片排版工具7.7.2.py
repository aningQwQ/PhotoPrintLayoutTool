...
feat: 可拓展的配置文件存储 (ConfigManager)，支持按 section 组织配置，工作目录持久化
feat: 排版设置实时保存与自动加载 (画布尺寸、间距、DPI、边距、排版模式、吸附对齐、导出格式)
"""
...
class ConfigManager:
    ...
    按 section 组织配置，支持任意扩展新的配置段。
    配置文件存储在程序工作目录下的 photo_layout_config.json。
    """
...
class EnhancedPhotoLayoutTool(QMainWindow):
    ...
    def _connect_layout_settings_save(self):
        signals = [
            ...
            self.row_packing_check.stateChanged,
    ...
    def _save_layout_settings(self):
        config_manager.set_section("layout_settings", {
            ...
            "row_packing": self.row_packing_check.isChecked(),
    ...
    def _load_layout_settings(self):
        ...
        if "row_packing" in settings:
            self.row_packing_check.setChecked(settings["row_packing"])
