...
feat: 完整实现双条带切割排版算法（Multi‑Sub‑Strip Packing），支持多尺寸混合排版且切割线连续
"""
    ...
    def double_strip_packing(self, rects):
        ...
        while remaining:
            ...
            if remaining_width > 0:
                sub_strip_y = row_y
                while True:
                    # 从 remaining 中选择一个宽度 <= remaining_width 的照片（优先宽度大的）
                    candidate = None
                    for r in remaining:
                    ...
                    sub_strip_height = max_height

                    # 找出 remaining 中所有与该 candidate 完全相同尺寸的照片（宽、高、slot_w、slot_h 完全相等）
                    same_size = [r for r in remaining
                                 if r['slot_w'] == candidate['slot_w'] and r['slot_h'] == candidate['slot_h']]

                    # 计算子条带内能堆叠的数量
                    max_fit = sub_strip_height // candidate['slot_h']
                    fit_count = min(len(same_size), max_fit)

                    if fit_count == 0:
                        break  # 无法堆叠任何照片（理论上不会）

                    # 放置这些照片（纵向堆叠）
                    placed_in_sub = []
                    for i in range(fit_count):
                        rect_to_place = same_size[i]
                        y_pos = sub_strip_y + i * rect_to_place['slot_h']
                        # 确保不超出页面高度
                        if y_pos + rect_to_place['height'] <= self.canvas_height:
                            sub_item = {
                                'x': used_width,
                                'y': y_pos,
                                'width': rect_to_place['width'],
                                'height': rect_to_place['height'],
                                'source_index': rect_to_place['src_idx'],
                                'rotation': False
                            }
                            placed_in_sub.append(rect_to_place)
                            add_item(sub_item)
                        else:
                            break  # 超出页面高度，剩余照片留给下一页
...
class EnhancedPhotoLayoutTool(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("证件照片排版工具v7.7.0")
