...
class EnhancedPhotoLayoutTool(QMainWindow):
    def __init__(self):
        ...
        self.photo_size_combo.addItems(["1寸 (2.5cm×3.5cm)", "2寸 (3.5cm×4.9cm)", "小1寸 (2.2cm×3.2cm)", 
                                       "大1寸 (3.3cm×4.8cm)", "5寸 (12.7cm×8.9cm)", "签证照片 (3.5cm×4.5cm)",
                                       "身份证照片 (3.5cm×5.3cm)", "护照照片 (3.5cm×4.5cm)", "自定义尺寸"])
        ...
        self.canvas_size_combo.addItems(["6寸(4R):(15.2cm×10.2cm)", "5寸(3R):(12.7cm×8.9cm)", 
                                        "7寸(5R):(17.8cm×12.7cm)", "A4:(21cm×29.7cm)", 
                                        "A5:(14.8cm×21cm)", "A6:(10.5cm×14.8cm)", "自定义尺寸"])
        ...
        self.orientation_horizontal = QRadioButton("横向")
        self.orientation_vertical = QRadioButton("竖向")
    ...
    def calculate_layout(self):
        ...
        if self.orientation_mode == 0:  # 自动
            # 横向布局计算
            cols_h, rows_h = self.calculate_rows_cols(canvas_width, canvas_height)
            count_h = cols_h * rows_h
            
            # 竖向布局计算
            cols_v, rows_v = self.calculate_rows_cols(canvas_height, canvas_width)
            ...
            orientation = "横向"
            cols, rows = self.calculate_rows_cols(canvas_width, canvas_height)
        else:  # 竖向
            orientation = "竖向"
            canvas_width, canvas_height = canvas_height, canvas_width
            cols, rows = self.calculate_rows_cols(canvas_width, canvas_height)
        ...
        canvas_w = self.cm_to_pixels(canvas_width, self.dpi)
        canvas_h = self.cm_to_pixels(canvas_height, self.dpi)
        ...
        return {
            ...
            'physical_photo': self.photo_size
        }
    
    def calculate_rows_cols(self, canvas_w, canvas_h):
        """计算给定方向下的行列数"""
        # 物理尺寸（厘米）
        photo_w, photo_h = self.photo_size
        spacing_w, spacing_h = self.spacing

        # 确保尺寸与间距之和大于0，并且将其转为整型
